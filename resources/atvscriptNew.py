...
sys.path.insert(0,'atvremote/lib/python3.7/site-packages')
from pyatv import connect, scan
from pyatv.const import Protocol
from pyatv.interface import Playing, RemoteControl, PushListener, retrieve_commands, PowerListener, DeviceListener
...
class PowerListener(PowerListener):
	...
	def powerstate_update(self, old_state, new_state):
		print(self.formatter(output(True, values={"PowerState": "{0:s}".format(new_state)})))

class DeviceListener(DeviceListener):
...
async def _handle_command(args, loop):
    ...
    if args.command == "push_updates":
        ...
        atv.power.listener = PowerListener(args.output)
